...
def get_file_from_blob(file_name):
    ...
    try:
        ...
        logging.info(f"Successfully downloaded blob '{file_name}'.")
        return blob_content
    except ResourceNotFoundError:
        logging.error(f"Blob '{file_name}' not found in container '{CONTAINER}'.")
        return None
    except Exception as e:
        logging.error(f"Failed to download blob '{file_name}': {e}", exc_info=True)
        return None
